...
def extract_account_identity(full_pdf_text):
    ...
    if not holder:
        ...
        skip_words = ["ROYAL BANK", "BANK OF MONTREAL", "TD", "BUSINESS BANKING", "BANQUE NATIONALE", "STATEMENT", "PAGE", "BRANCH"]
        for l in lines:
            if "_" in l: continue 
            if re.match(r'^\d', l) or " ST " in l.upper() or " AVE " in l.upper() or " ON " in l.upper() or " BC " in l.upper(): continue 
            if len(l) > 3 and l.upper() == l and not any(sw in l.upper() for sw in skip_words) and not re.match(r'^[\d\s\-\.\$,]+$', l):
                holder = l
                break
    
    m = re.search(r'Account\s+No\.?\s*\n*([\d\s\-]{6,})', full_pdf_text, re.IGNORECASE)
    if not m: m = re.search(r'Account\s+number:?\s*\n*([\d\s\-]{6,})', full_pdf_text, re.IGNORECASE)
    if m: acct_num = re.sub(r'\D', '', m.group(1))
...
def extract_opening_closing_balance(full_pdf_text):
    ...
    m = re.search(r'(?:Opening\s+balance|BALANCE\s+FORWARD)\s*(?:on[^\d\n]*)?[:\s]*\$?\s*([\-\d,]+\.\d{2})', full_pdf_text, re.IGNORECASE)
    if m:
...
# ==============================================================================
with tab1:
    st.header("1. Document Analyzor")

    col_u1, col_u2 = st.columns(2)
    with col_u1:
        uploaded_files = st.file_uploader("Upload Bank Statements (PDF)", type="pdf", accept_multiple_files=True)
    with col_u2:
        credit_file = st.file_uploader("Upload Credit Report (PDF)", type="pdf", accept_multiple_files=False)
    ...
    if st.button("🚀 Process Statements", type="primary") and uploaded_files:
        ...
        for f_idx, f in enumerate(uploaded_files):
            ...
            with pdfplumber.open(io.BytesIO(pdf_bytes)) as doc:
                ...
                if anchor_method:
                    ...
                    st.session_state.diagnostic_log.append(
                        f"↪️ {f.name}: No summary total detected"
                ...
                for page_num, page in enumerate(doc.pages):
                    ...
                    for yk_idx, y_key in enumerate(sorted_y_keys):
                        ...
                        # TD (DILUTED MONTHS FIX) 
                        if any(k in full_line.upper() for k in ["ACCOUNT/TRANSACTION TYPE", "FEES PAID", "NEXT STATEMENT", "MONTHLY AVER", "DEP CONTENT", "CHQS ENCLOSED"]):
                            break
        ...
        if len(entities_seen) > 1:
            ...
            st.session_state.entity_warning = (
                ...
                f"account holders: {entity_summary}. Mixing entities will corrupt revenue, "
                f"concentration %, and the reconciliation check below."
            )

        if skipped_duplicate_files:
            st.warning(f"⚠️ Skipped {len(skipped_duplicate_files)} duplicate file(s): {', '.join(skipped_duplicate_files)}")

        if not all_deposits:
            st.error(" No deposits found. Please verify PDF format")
        else:
            ...
            if anchor_source_count < len(uploaded_files) - len(skipped_duplicate_files):
                st.warning("⚠️ Extraction complete, but one or more files had no verifiable total.")
            else:
    ...
    if st.session_state.transactions is not None and not st.session_state.transactions.empty:
        ...
        if expected_credits == 0.0:
            ...
            st.warning("⚠️ Anchor Warning: No summary total could be found for any file.")
        elif variance > 2.00:
            ...
            st.success("✅ Reconciliation Passed: extracted deposits match the resolved total(s).")
